...
namespace MathAndProcess.Calculations
{
    public class ResemplingOfSignal
    {
        ...
        private static List<double> resampling_lagrange(List<double> inputData, double p, double q, double x0)
        {
            ...
            inputData.Add(0);
            inputData.Add(0);
            List<double> resample_data = new List<double>();
            List<double> t = new List<double>();
            for (int k = 0; k < ind - 1; k++)
            {
                double x = k * q / p - x0;
                t.Add(x);
                int n = (int)(Math.Floor(x)) + 4;
                double d = Math.Floor(x) + 1 - x;
                double a0 = inputData[n - 1];
                double a3 = 1 / 6 * (inputData[n] - inputData[n - 3]) + 0.5 * (inputData[n - 2] - inputData[n - 1]);

                double a1 = 0.5 * (inputData[n] - inputData[n - 2]) - a3;
                double a2 = inputData[n] - inputData[n - 1] - a3 - a1;
                //var b = Math.Pow(d,2);
        ...
        private static List<double> resampling_lagrange1(List<double> inputData, double p, double q, double x0)
        {
            ...
            for (int k = 0; k < ind - 1; k++)
            {
                ...
                double a1 = -1.0 / 12 * inputData[n - 4] + 0.5 * inputData[n - 3] - 3.0 / 2 * inputData[n - 2] + 5.0 / 6 * inputData[n - 1] + 1.0 / 4 * inputData[n];
                double a2 = -1.0 / 24 * inputData[n - 4] + 1.0 / 6 * inputData[n - 3] + 1.0 / 4 * inputData[n - 2] - 5.0 / 6 * inputData[n - 1] + 11.0 / 24 * inputData[n];
                double a3 = 1.0 / 12 * inputData[n - 4] - 0.5 * inputData[n - 3] + 1 * inputData[n - 2] - 5.0 / 6 * inputData[n - 1] + 1.0 / 4 * inputData[n];
                double a4 = 1.0 / 24 * inputData[n - 4] - 1.0 / 6 * inputData[n - 3] + 1 / 4.0 * inputData[n - 2] - 1.0 / 6 * inputData[n - 1] + 1.0 / 24 * inputData[n];
                //var b = Math.Pow(d,2);
